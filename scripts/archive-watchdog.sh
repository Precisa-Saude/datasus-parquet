...
REPO_DIR=/Users/rafael/Github/precisa-saude/datasus-parquet
...
start_archive() {
  ...
  NODE_OPTIONS="--max-old-space-size=8192" pnpm archive-sia-pa -- \
    --ufs ALL --years "$YEARS" --throttle-ms 100 --year-pause-ms 0 \
    > "$ARCHIVE_LOG" 2>&1 &
  echo $!
}

log "===== Watchdog start (years=$YEARS, stall=${STALL_THRESHOLD_MIN}min, max_retries=$MAX_RETRIES) ====="
